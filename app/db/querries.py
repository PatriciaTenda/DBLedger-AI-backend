    ...
    if conn: 
        ...
        try:
            with conn.begin():# Utilisation de transaction sécurisée
                ...
                print("Table invoice créée avec succès !")
    ...
    if conn: 
        try:
            with conn.begin():
                ...
                content_length,
                content_md5 
                ) values (:id_invoice, :name_invoice, :created_at, :modified_at, :content_length, :content_md5)
            ...
            print(f"Erreur lors de lors de l'enregistrement des données dans la table invoices, {e}")
...
def check_invoice_exists(invoice_id):
    ...
    try:
        with conn.begin():
            ...
            query = "SELECT COUNT(*) FROM t_invoice WHERE id_invoice = :invoice_id"
